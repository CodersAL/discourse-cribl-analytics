class ::CriblAnalytics::AnalyicsController < ::ApplicationController
  PAGE_SIZE = 50
  ...
  def index
    ...
    case params[:period]
    when 'all_time'
      success, error, result = ::CriblAnalytics::PostgresQueries.all_time(params[:user_id] || nil)
    when 'today'
      success, error, result = ::CriblAnalytics::PostgresQueries.today(params[:user_id] || nil)
    else
      success, error, result = ::CriblAnalytics::PostgresQueries.quarter(params[:user_id] || nil)
    end
  ...
  private def build_load_more_uri(params)
    ...
    load_more_uri = URI.parse(analytics_path(more_params))
  ...
  private def build_response(params, success, error, result)
    if success
      ...
      render_json_dump(data: data,
                       meta: {
                         total_rows_analytics: result_count,
                         load_more_analytics: build_load_more_uri(params)
                       }
      )
    else
