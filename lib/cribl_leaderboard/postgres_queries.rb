...
class ::CriblAnalytics::PostgresQueries
  ...
  def self.today(user_id)
    get_data = Proc.new {|id| id.nil? ? AnalyicsTodaysRankingView.all : AnalyicsTodaysRankingView.where({id: id})}
    self.return_results(user_id, get_data, "cribl_today_ranking")
  end


  def self.quarter(user_id)
    get_data = Proc.new {|id| id.nil? ? AnalyicsQuarterRankingView.all : AnalyicsQuarterRankingView.where({id: id})}
    self.return_results(user_id, get_data, "cribl_quarter_ranking")
  end


  def self.all_time(user_id)
    get_data = Proc.new {|id| id.nil? ? AnalyicsTotalRankingView.all : AnalyicsTotalRankingView.where({id: id})}
